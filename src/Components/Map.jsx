...
const Map = ({ ranges, selectedRange }) => {
    ...
    if (mapRef.current) {
      ...
      ranges.forEach((range) => {
        if (range.latitude && range.longitude) {
          ...
          root.render(<FaMapMarkerAlt style={{ color: "red", fontSize: "24px" }} />);
          ...
          const marker = L.marker([range.latitude, range.longitude], { icon: customIcon })
            .addTo(map)
  ...
  useEffect(() => {
    if (mapRef.current && selectedRange) {
      mapRef.current.setView([selectedRange.latitude, selectedRange.longitude], 12);
    }
  }, [selectedRange]);
