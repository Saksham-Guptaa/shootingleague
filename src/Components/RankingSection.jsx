...
const RankingsSection = () => {
  ...
  useEffect(() => {
    ...
    fetch('/athletes.json')
      ...
      .then(data => {
        const men = data.filter(athlete => athlete.gender === "Male").map(athlete => ({
          name: athlete.name,
        ...
        const women = data.filter(athlete => athlete.gender === "Female").map(athlete => ({
          name: athlete.name,
          event: athlete.achievements[0]?.event || "Not specified",
          img: athlete.image || "/images/default.jpg",
          country: athlete.country || "Not specified",
  ...
  const calculateTotalMedals = (medals) => {
    ...
    Object.values(medals).forEach(category => {
      if (category) {
        total += (category.gold || 0) + (category.silver || 0) + (category.bronze || 0);
      }
    });
  ...
  useEffect(() => {
    ...
    const filtered = athletes.men.filter(athlete => 
  ...
  useEffect(() => {
    ...
    const filtered = athletes.women.filter(athlete => 
        ...
        <div>
          ...
          <div className="mt-4 space-y-4">
            {filteredMen.length > 0 ? (
              ...
              <p className="text-center py-4">No athletes found matching your search.</p>
            )}
          </div>
        </div>
